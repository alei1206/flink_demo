...
public class t4_broadcast {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //将流转换为广播流，需要传一个MapStateDescriptor参数，就是一个广播状态描述广播流的key类型与value类型
        //此时，key就是一个string，value就是一个二元组。
        //注意，这个key与value不能随便写，后面需要通过key与另外的流进行关联。
        //另外需要传TypeInformation，因为在运行时，泛型会擦除
        MapStateDescriptor<String, Tuple2<String, String>> userInfoStateDesc = new MapStateDescriptor<>(
                "userInfoStateDesc",
                TypeInformation.of(String.class),   //key的类型提示
                TypeInformation.of(new TypeHint<Tuple2<String, String>>() {}));  //value的类型提示
